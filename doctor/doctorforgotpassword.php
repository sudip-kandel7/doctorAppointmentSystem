...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $mysqli->prepare("SELECT id FROM doctor WHERE email = ?");
    ...
    if ($doctor) {
        ...
        // Save to DB
        $update = $mysqli->prepare("UPDATE doctor SET reset_token=?, reset_token_expiry=? WHERE id=?");
        $update->bind_param("ssi", $token, $expiry, $doctor['id']);
        $update->execute();

        // Show reset link (in real life you’d email this)
        $resetLink = "http://localhost/DOCTORAPPOINTMENTWEB/doctor/doctorreset_password.php?token=$token";
